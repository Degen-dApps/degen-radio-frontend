...
export function swapTokens(signer, receiver, inputToken, outputToken, amountIn, amountOutMin, routerAddress, referrer) {
  ...
  const wrappedInterface = new ethers.utils.Interface(['function deposit() payable', 'function withdraw(uint wad)'])
  ...
  if (inputTokenAddress === ethers.constants.AddressZero && outputTokenAddress === wrappedAddress) {
    ...
    if (
      ...
      if (referrer === ethers.constants.AddressZero) {
        ...
        return routerContract.swapExactETHForTokensWithReferrer(amountOutMin, path, receiver, deadline, referrer, {
          value: amountIn,
        })
      }
    } else if (
      ...
      if (referrer === ethers.constants.AddressZero) {
        ...
        return routerContract.swapExactTokensForETHWithReferrer(
          amountIn,
          amountOutMin,
          path,
          receiver,
      ...
      if (referrer === ethers.constants.AddressZero) {
        ...
        return routerContract.swapExactTokensForTokensWithReferrer(
          amountIn,
          amountOutMin,
          path,
          receiver,
